...
function validateSessionTime(){
	var start = document.forms['myForm']['sessionStart'].value;
	var end = document.forms['myForm']['sessionEnd'].value;
	
	var sessionStart = document.forms['myForm']['sessionStart'];
	var sessionEnd = document.forms['myForm']['sessionEnd'];

	var sessionStartErr = document.getElementById("sessionStartErr");
	var sessionEndErr = document.getElementById("sessionEndErr");
	
	if(start.length == 0 ){
		sessionStartErr.innerHTML = "*Start Time is required";
		...
		sessionEndErr.style.display = 'none';
		return false;
	}

	if(end.length == 0 ){
		sessionEndErr.innerHTML = "*End Time is required";
		...
		sessionStartErr.style.display = 'none';
		return false;
	}

	if( (start >= "10:00" && start <= "16:00") || (start >= "18:00" && start <= "21:00") ){

		if( (end >= "10:00" && end <= "16:00") || (end >= "18:00" && end <= "21:00") ){
...
if(x == 's'){
	...
	if(!validateSessionTime()) {error = 1;}
